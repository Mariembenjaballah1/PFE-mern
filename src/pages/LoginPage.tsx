...
const LoginPage: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Just set visual indicators if needed
      if (error.message === 'Email incorrect') {
        setEmailError('Email incorrect');
      } else if (error.message === 'Mot de passe incorrect') {
        setPasswordError('Mot de passe incorrect');
      } else if (error.message !== 'Échec de la connexion') {
        // Only show fallback behavior if it's not a handled error
        setTimeout(() => {
          navigate(from);
        }, 100);
      }
        ...
        <Card className="backdrop-blur-md bg-white/80 border-none shadow-xl">
          <CardHeader className="space-y-1 flex flex-col items-center pb-6">
            ...
            <CardDescription className={`text-center transition-all duration-700 delay-500 transform ${animationComplete ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
              Connectez-vous à votre compte InvenTrack
            </CardDescription>
          </CardHeader>
          ...
          <CardFooter className="flex flex-col space-y-4 pt-0">
            <div className={`text-sm text-center text-muted-foreground transition-all duration-700 delay-1300 transform ${animationComplete ? 'translate-y-0 opacity-100' : 'translate-y-4 opacity-0'}`}>
              Pour la démo utilisez: admin@example.com / password
            </div>
