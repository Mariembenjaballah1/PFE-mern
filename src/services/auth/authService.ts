...
export const loginWithRefreshToken = async (email: string, password: string): Promise<LoginResponse> => {
  ...
  try {
    ...
    const response = await api.post('/auth/login-with-refresh', { email, password });
    ...
    try {
      ...
      // For development/demo purposes, still allow login but show warning
      ...
      toast.success("Connexion en mode démo", {
        description: "Bienvenue sur InvenTrack (Mode Développement)"
      });
...
export const isTokenExpired = (token: string): boolean => {
  try {
    ...
    if (token.startsWith('mock-jwt-token')) {
      // Mock tokens are valid for 1 hour
      const timestamp = parseInt(token.split('-').pop() || '0');
      const now = Date.now();
      const isExpired = (now - timestamp) > 3600000; // 1 hour
      console.log('Mock token expiration check:', { timestamp, now, isExpired });
      return isExpired;
    }
